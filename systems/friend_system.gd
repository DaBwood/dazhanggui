...
func chat_with_friend(once: bool = true) -> Dictionary:
	...
	if once:
		...
		var bond_gain = int(round(f.talent * (1.0 + g.collection_system.get_chat_bond_pct() + g.collection_system.get_friend_bond_category_pct(f.category) + g.talent_system.get_friend_chat_bond_pct(f.category) / 100.0)))   # 【修】2026-09-25 口径修复：天赋 getter 返回百分数（5=5%），藏品两项本已 /100，本项漏除致谈心缘分 100 倍   # 【改】三批+四批：凝冰套装%+职业单品%（c071/073/079/081）
		f.bond += bond_gain
